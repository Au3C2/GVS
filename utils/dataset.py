...
class LiverDatasetEval(Dataset):
    def __init__(self, imgs_dir, other_root):
        '''
        other_root means the img from other model, like:
        /home/lx/RESULT/antgan_brats/
        '''
        self.imgs_dir = imgs_dir
        ...
        self.img_list = img_list
        self.ori_root = '/'.join(img_list[0].split('/')[0:-2])
        # self.idx = 0
    ...
    def __getitem__(self,idx):
        ...
        min1 = img.min()
        max1 = img.max()
        img_norm = (img-min1)/(max1-min1)

        img1 = torch.tensor(img)
        img1_norm = torch.tensor(img_norm)
        seg = torch.tensor(seg)
        seg_tumor = torch.tensor(seg_tumor)

        img2_path = self.img_list[idx].replace(self.ori_root,self.other_root)
        data = np.load(img2_path).item()
        img = data['liver']
        body_mask = np.where(img==img[0,0],0,1)
        min1 = img.min()
        max1 = img.max()
        img_norm = (img-min1)/(max1-min1)

        img2_norm = torch.tensor(img_norm)
        img2 = torch.tensor(img)

        return img1, img1_norm, img2, img2_norm, seg, seg_tumor, case_name, slice_idx

class BrainDatasetEval(Dataset):
    def __init__(self, imgs_dir, other_root):
        '''
        other_root means the img from other model, like:
        /home/lx/RESULT/antgan_brats/
    ...
    def __getitem__(self,idx):
        ...
        min1 = img.min()
        max1 = img.max()
        img_norm = (img-min1)/(max1-min1)*brain_mask
        ...
        min2 = img.min()
        max2 = img.max()
        img_norm = (img-min2)/(max2-min2)*brain_mask
        img2 = torch.tensor(img)
        img2_norm = torch.tensor(img_norm)

        return img1, img1_norm, img2, img2_norm, seg, seg_tumor, case_name, slice_idx, mean, std
