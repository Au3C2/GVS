...
def train_net(reconstucter,
    ...
    train_set = BrainDataset(
        train_list, scale_seg=args.scale_seg, percent=percent)
    test_set = BrainDataset(test_list)
    train_loader = DataLoader(train_set, batch_size=batch_size,
                              shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    test_loader = DataLoader(
        test_set, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True)
    n_train = len(train_loader)
    n_test = len(test_loader)

    logger.info(f'''Starting training:
        File names       brats_train.py
    ...
    optimizer_R = optim.Adam(reconstucter.parameters(),
                             lr=lr, betas=(0.9, 0.99))
    optimizer_S = optim.Adam(segmenter.parameters(), lr=lr, betas=(0.9, 0.99))
    scheduler_R = optim.lr_scheduler.StepLR(
        optimizer_R, step_size=int(0.8*epochs))
    scheduler_S = optim.lr_scheduler.StepLR(
        optimizer_S, step_size=int(0.8*epochs))
    ...
    for epoch in range(epochs):
        ...
        bar = tqdm(enumerate(train_loader), total=n_train,
                   desc=f'Epoch {epoch+1}/{epochs}', unit='batch', ncols=120, ascii=True)
        for i, data in bar:
            [img, seg, seg_tumor, brain_mask, case_name, slice_idx] = data[0:6]
            img = Variable(img.unsqueeze(dim=1).to(
            ...
            seg_no_tumor = torch.zeros_like(seg).to(
            ...
            loss_mse = lambd[2] * (mse_loss(img_pred, img)
                                   * seg_tumor_neg).mean()
            loss_var = lambd[3] * (mse_loss(img_pred, brain_avg.to(
                dtype=torch.float)).squeeze(1) * seg_tumor).mean()
            ...
            diff_norm = diff / \
                diff.view(batch_size, -1).max(dim=-1)[0].view(batch_size, 1, 1)
            weightt = 1 - diff_norm
            weightt = torch.where(weightt < 0.1, 0.1 *
                                  torch.ones_like(weightt), weightt)

            loss_seg1 = lambd[0] * \
                (ce_loss1(seg_pred, seg_tumor) * weightt).mean()
            loss_seg1.backward()
            optimizer_S.step()

            bar.set_postfix_str('loss(batch):{:>.2e},{:>.2e},{:>.2e},{:.2e}'.format(
                loss_seg1.item(), loss_seg2.item(), loss_mse.item(), loss_var.item()))
            loss1 += loss_seg1
            loss2 += loss_seg2
            loss3 += loss_mse
            loss4 += loss_var
        logger.info('Epoch loss: {:.2e}, {:.2e}, {:.2e}, {:.2e}'.format(
            loss1/i, loss2/i, loss3/i, loss4/i))
        scheduler_S.step()
        scheduler_R.step()
...
def get_args():
    ...
    parser.add_argument('-s', '--scale', dest='scale_seg',
                        default=0, type=int, help='scale of tumor seg lable')
    parser.add_argument('-g', '--gpu', dest='gpu', default=0,
                        type=int, help='witch gpu to use')
    ...
    parser.add_argument('-a', '--architecture', dest='arch', type=str, default='Segmenter',
                        help='Architecture of segmentor')
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.error('User canceled, start on %s, Saved interrupt.' %
                     (starttime_r))
